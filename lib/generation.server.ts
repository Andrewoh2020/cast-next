...
export const PROFILE_FROM_UPLOAD_PROMPT =
  `Recreate this person as a professional studio portrait. Keep their face, skin tone, hair, body type, age, and proportions IDENTICAL to the reference image — this must clearly be the same person, no facial changes, no rejuvenation, no reshaping. Medium shot framing, cropped at the knees, head-to-knees only. Plain seamless warm-gray studio backdrop, no environment, no props. Body turned slightly toward camera, relaxed natural pose, neutral confident expression. Photorealistic DSLR, 85mm lens, soft diffused studio lighting, sharp skin detail, no retouching, no airbrushing. Do NOT show feet or ankles.

REPLACE the subject's wardrobe with neutral casting-profile attire: a fitted plain heather-grey, charcoal, or black crewneck t-shirt or simple knit top, paired with plain dark trousers (charcoal, navy, or black). The wardrobe must be plain and unbranded. Do NOT preserve from the reference image: graphic prints, brand logos, character/mascot artwork, slogans or text on clothing, athletic wear, basketball shorts, gym shorts, tank tops, sleeveless tops, hoodies, sleepwear, beachwear, or any casual home-wear — these are unsuitable for a casting profile. Only the clothing is restyled; the face, body, hair, and identity remain identical to the reference.`;
